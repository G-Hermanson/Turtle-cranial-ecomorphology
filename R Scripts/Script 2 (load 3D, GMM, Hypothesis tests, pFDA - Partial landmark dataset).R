...
#get corrected p-values using FDR
  p.corr.partial <- lapply( lapply ( lapply ( procD.fit.partial , get.procD.p) , 
...
scores.all$sizes <- log10(size.partial)[rownames(scores.all)]
  ...
  #D-PGLS with "neck ratio" info using partial landmark dataset
  
  extant_tree_partial_neck <- keep.tip(extant_tree_partial,taxa_neck_info)
  
  coords.neck.partial <- GPA.partial$coords
  dimnames(coords.neck.partial)[[3]] <- sort(partial_taxa)
  coords.neck.partial <- coords.neck.partial[,,extant_tree_partial_neck$tip.label]
  
  gdf.neck.partial <- geomorph.data.frame( "shape" = coords.neck.partial,
                                           "skull_size" = log10(size.partial)[extant_tree_partial_neck$tip.label] ,
                                           "feeds_on_water" = turtle_data[extant_tree_partial_neck$tip.label,"Feed_on_water"] , 
                                           "feeds_on_land" = turtle_data[extant_tree_partial_neck$tip.label,'Feed_on_land'] , 
                                           "suction" = turtle_data[extant_tree_partial_neck$tip.label,'Suction_feeding'],
                                           "durophagous" = turtle_data[extant_tree_partial_neck$tip.label,'Mostly_hard_food..durophagy.'],
                                           "plant" = turtle_data[extant_tree_partial_neck$tip.label,'Mostly_vegetable_matter..herbivory.'],
                                           "meat" = turtle_data[extant_tree_partial_neck$tip.label,'Mostly_animal_matter..carnivory.'] ,
                                           "marine" = turtle_data[extant_tree_partial_neck$tip.label,'Marine'] ,
                                           "swim" = turtle_data[extant_tree_partial_neck$tip.label,'Open_swimmer'],
                                           "neck_retraction" = turtle_data[extant_tree_partial_neck$tip.label,'Neck_retraction'],
                                           "pleurodira" = turtle_data[extant_tree_partial_neck$tip.label,'Side_neck'],
                                           "hardness" = hardness_index[extant_tree_partial_neck$tip.label],
                                           "evasiveness" = evasiveness_index[extant_tree_partial_neck$tip.label],
                                           "neck_ratio" = turtle_data[extant_tree_partial_neck$tip.label,"Neck_ratio"],
                                           "phy" = extant_tree_partial_neck )
  ...
  right.sides.neck.partial <- c("skull_size",
                                ...
                                "skull_size+suction+durophagous+neck_retraction+evasiveness",
                                "skull_size+suction+durophagous+neck_retraction+evasiveness+neck_ratio",
                                "skull_size+suction+durophagous+neck_retraction+neck_ratio")

models.neck.partial <- paste( "shape ~" , right.sides.neck.partial )
models.neck.partial <- lapply( models.neck.partial , as.formula )

# Run D-PGLS 

procD.fit.neck.partial <- list()

for( i in 1:length(models.neck.partial) ) {
  procD.fit.neck.partial[[ i ]] <- procD.pgls( models.neck.partial[[ i ]] , data = gdf.neck.partial , phy = phy , SS.type = "II" )
  names(procD.fit.neck.partial)[[i]] <- as.character(models.neck.partial[i])
  
}

#inspect results
lapply( procD.fit.neck.partial , summary )

#get corrected p-values using FDR
p.corr.neck.partial <- lapply( lapply ( lapply ( procD.fit.neck.partial , get.procD.p) , 
                                        p.adjust, method="fdr") , round , 4)
...
models.R2.neck.partial <- sort(unlist(lapply(procD.fit.neck.partial,get.R2)),decreasing = T)

model.temp.neck.partial <- names (models.R2.neck.partial)[4] #change number in brackets until conditions above are met
procD.fit.neck.partial[[model.temp.neck.partial]]$aov.table  #check

bestmodel.neck.partial <- procD.fit.neck.partial[[model.temp.neck.partial]] ## best model for full dataset

#In this case, the best model using this smaller dataset (N=65) included
#the same variables as the one with greater sample size (N=76)

# save and export results      
turtle.aov.neck.partial <- do.call( what = rbind ,
                            arg = lapply( procD.fit.neck.partial[order(models.R2.neck.partial,decreasing = T)] , 
                                          function( X ) { X$aov.table } ) )

write.csv(turtle.aov.neck.partial , "turtle_aov_partial_neck.csv" )
...
#Optional step
  ...
  #To run it using the topology from Sterli et al. (2018), add the '#' prior to line 318, and remove the '#' prior to line 319;
